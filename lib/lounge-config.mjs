...
// Keep the one-shot end timer below the browser's signed 32-bit millisecond limit.
export const MAX_PILOT_WINDOW_MS = 2_147_000_000;
...
export function getLoungeConfig(environment = process.env) {
  ...
  if (!Number.isFinite(startTime) || !Number.isFinite(endTime)
    || windowMs <= 0 || windowMs >= MAX_PILOT_WINDOW_MS) {
    return { mode: "closed", reason: "invalid_schedule" };
  }
...
/** Return a browser-safe delay for the next lounge timer callback. */
export function getLoungeTimerDelay(endTime, now = Date.now()) {
  return Math.min(MAX_PILOT_WINDOW_MS, Math.max(0, endTime - now));
}
 ...
 * @returns {"closed" | "scheduled" | "consent" | "open"}
 */
export function getLoungeViewState(config, now, consented) {
  if (config.mode === "closed") return "closed";
  if (now === null || !isLoungeOpen(config, now)) return "scheduled";
  return consented ? "open" : "consent";
}
