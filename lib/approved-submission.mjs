...
export function buildPublicSolution(fields, { thumbnail = null } = {}) {
  ...
  const hasRawTypesAndUses = Boolean(rawTypesAndUses);
  const q6 = hasRawTypesAndUses
    ? normalizeTypesAndUses(rawTypesAndUses)
    : fields.CatalogType || fields.CatalogCategories || fields.CatalogTags
    ? { status: "ok", type: null, categories: [], tags: [] }
    : normalizeTypesAndUses(fields.TypesAndUses);
  ...
  const hasRawRelatedUrls = Boolean(rawRelatedUrls);
  ...
  const solution = {
    ...
    type: hasRawTypesAndUses ? q6.type : fields.CatalogType ? requiredText(fields, "CatalogType") : q6.type,
    categories: hasRawTypesAndUses ? q6.categories : fields.CatalogCategories ? lines(fields.CatalogCategories) : q6.categories,
    tags: hasRawTypesAndUses ? q6.tags : fields.CatalogTags ? lines(fields.CatalogTags) : q6.tags,
    distributionUrl: optionalHttpsUrl(fields, "DistributionUrl"),
    sourceUrl: hasRawRelatedUrls ? q8.sourceUrl : fields.SourceUrl ? optionalHttpsUrl(fields, "SourceUrl") : q8.sourceUrl,
    instructionsUrl: hasRawRelatedUrls ? q8.instructionsUrl : fields.InstructionsUrl ? optionalHttpsUrl(fields, "InstructionsUrl") : q8.instructionsUrl,
    license: fields.CatalogLicense ? requiredText(fields, "CatalogLicense") : "配布先を確認",
    cost: fields.CatalogCost ? requiredText(fields, "CatalogCost") : "配布先を確認",
    premiumRequired: fields.PremiumRequired ? premiumRequired(fields.PremiumRequired) : null,
    setupTime: fields.SetupTime ? requiredText(fields, "SetupTime") : "未記載",
    prerequisites: fields.CatalogPrerequisites ? lines(fields.CatalogPrerequisites) : [],
    thumbnail,
