...
export async function prepareApprovedSubmission({
  ...
  const stableReviewDate = preparedFields.ReviewedAt
    || preparedFields.SubmittedAt
    || new Date().toISOString();
  if (!String(preparedFields.CatalogPublishedDate ?? "").trim()) {
    preparedFields.CatalogPublishedDate = stableReviewDate;
  }
  if (!String(preparedFields.CatalogUpdatedDate ?? "").trim()) {
    preparedFields.CatalogUpdatedDate = preparedFields.CatalogPublishedDate;
  }
