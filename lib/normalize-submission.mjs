...
export function normalizeRelatedUrls(value) {
  ...
  if (Array.isArray(value)) {
    lines = value.map(String).map((v) => v.trim()).filter(Boolean);
  } else {
    const text = String(value ?? "").trim();
    if (text.startsWith("[") && text.endsWith("]")) {
      try {
    ...
    if (!lines) lines = text.split(/\r?\n/).map((v) => v.trim()).filter(Boolean);
  }
  if (!lines.length) return { status: "ok", sourceUrl: null, instructionsUrl: null, relatedUrls: [] };

  const result = { sourceUrl: null, instructionsUrl: null, relatedUrls: [] };
  const labeled = lines.map((line) => /^(ソース|手順):\s*(\S+)$/.exec(line));
  if (labeled.some(Boolean) && labeled.some((match) => !match)) return { status: "要確認" };

  const seen = new Set();
  for (const line of lines) {
    const match = /^(ソース|手順):\s*(\S+)$/.exec(line);
    const candidate = match ? match[2] : line;
    let url;
    try { url = new URL(candidate); } catch { return { status: "要確認" }; }
    if (url.protocol !== "https:") return { status: "要確認" };
    if (match) {
      if (seen.has(match[1])) return { status: "要確認" };
      seen.add(match[1]);
      result[match[1] === "ソース" ? "sourceUrl" : "instructionsUrl"] = url.href;
    }
    result.relatedUrls.push(url.href);
  }

  if (!labeled.some(Boolean)) {
    result.sourceUrl = result.relatedUrls[0] ?? null;
    result.instructionsUrl = result.relatedUrls.length > 1 ? result.relatedUrls.at(-1) : null;
  }
