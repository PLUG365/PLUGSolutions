...
The flow intentionally ignores the Microsoft Forms `responder` property. The current form has nine questions: Q6 carries multi-select format/use values, Q8 carries at most one `ソース:` and one `手順:` HTTPS URL, and Q9/I01 carries the optional thumbnail candidate. Legacy P08/`Requirements` is retained only for backup and is not used for new public JSON. Only the public author name, public X handle, submitted work information, consent answer, response identifier, and submission timestamp are written to the private review list.
