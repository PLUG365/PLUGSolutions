...
- 作品ページのボタンから Cloudflare Workers のリアクション API を呼び出す。サインイン、コメント、ユーザーアカウントは要求しない。
- Worker は Cloudflare D1 Free に対象 slug、種別、SHA-256 化した匿名 visitor token、UTC日付だけを保存し、生の token、氏名、メール、X アカウントは保存しない。
- D1 の一意制約（slug × 種別 × token hash）と同一トランザクションで二重計上を防止し、日次受付は 5,000 件で hard cap とする。
- ブラウザの `localStorage` は同一端末での誤連打防止にだけ使用する。利用できない場合は送信ボタンを無効にし、不正防止や本人識別には使用しない。
- 公開時は人数ではなく「リアクション数」と表記する。
- 初期表示は `catalog/reactions.json` を使い、API が利用できる場合だけ作品ページと一覧をライブ集計へ更新する。
- `catalog/manifest` と D1 の `solution_slugs` を定期同期し、存在しない slug や未定義の種別は受け付けない。CORS は本番サイトの固定 origin だけを許可する。
- API 障害、D1 障害、上限到達時は fail-closed とし、サイトに個別回答や token hash を公開しない。
...
| リアクション回答 | なし → D1非公開記録 | Cloudflare Worker＋D1 |
| 同一リアクションの再送 | 既存 → 変更なし | Workerの一意制約 |
| リアクション集計 | D1の集計 → APIレスポンス | Worker（公開は集計値のみ） |

- JSON スキーマ、必須項目、HTTPS URL、slug 形式、重複 slug、日付、公開禁止フィールドを自動検証する。
- Forms 障害、SharePoint 障害、フロー再試行でも掲載データの欠落・二重登録が起きないことを確認する。
...
- 同じ visitor token の再送が二重計上されず、日次5,000件を超えて受け付けないことをWorkerテストとローカルD1で確認する。
- APIレスポンスと公開リポジトリに生token、token hash、個別回答、回答日時、IP、その他の非公開フィールドが含まれないことを確認する。
